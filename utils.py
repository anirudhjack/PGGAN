...
class CelebA(object):
    ...
    def getShapeForData(self, filenames, resize_w=256):
        array = [get_image(batch_file, 128, is_crop=False, resize_w=resize_w,
...
def get_image(image_path , image_size, is_crop=False, resize_w=32, is_grayscale=False):
...
def transform(image, npx=256 , is_crop=False, resize_w=64):
...
def read_image_list(category):
    filenames = []
    print("list file")
    #smb_fs = fs.open_fs('smb://lfovia:lfovia123@http://192.168.130.151:5000/Parimala')
    #smb_fs = fs.open_fs('smb://ds1817/ds_share/Parimala/All_Image_datasets/')
